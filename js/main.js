...
table.addEventListener('click', (e) => {
	...
	if (shift === "X") validate(target, dataset, player1, shift, "X")
	else validate(target, dataset, player2, shift, "O");
});

const tryAgain = () => window.location.reload();

const validate = (target, dataset, player, shiftParam, playerNum) => {
	target.textContent = shiftParam;
	player[parseInt(dataset)] = "i";
